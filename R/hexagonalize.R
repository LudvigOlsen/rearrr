...
#' @title Create x-coordinates such that points form a hexagon
#' @description
...
#' @param .min Minimum y-coordinate of the hexagon. If \code{NULL}, it is inferred by the given y-coordinates.
#' @param .max Maximum y-coordinate of the hexagon. If \code{NULL}, it is inferred by the given y-coordinates.
...
hexagonalize_mutator_method <- function(data, cols, .min, .max, offset_x,
  ...
  # Assign to left side on x-axis
  top[top[[tmp_side_col]] == 1, x_col_name] <-
    min_max_scale(
      top[top[[tmp_side_col]] == 1, col, drop = TRUE],
      new_min = 0,
      new_max = width / 2,
  ...
  # Assign to right side on x-axis
  top[top[[tmp_side_col]] == 2, x_col_name] <-
    min_max_scale(
      top[top[[tmp_side_col]] == 2, col, drop = TRUE],
      new_min = width,
      new_max = width / 2,
      old_min = middle_upper,
      old_max = .max
  ...
  middle[[x_col_name]] <- ifelse(middle[[tmp_side_col]] == 1, 0, width)

  # Bottom section

  # Assign to left side on x-axis
  bottom[bottom[[tmp_side_col]] == 1, x_col_name] <-
    min_max_scale(
      bottom[bottom[[tmp_side_col]] == 1, col, drop = TRUE],
      new_min = width / 2,
      new_max = 0,
      old_min = .min,
  ...
  # Assign to right side on x-axis
  bottom[bottom[[tmp_side_col]] == 2, x_col_name] <-
    min_max_scale(
      bottom[bottom[[tmp_side_col]] == 2, col, drop = TRUE],
      new_min = width / 2,
      new_max = width,
      old_min = .min,
      old_max = middle_lower
