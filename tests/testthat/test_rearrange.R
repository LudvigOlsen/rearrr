...
# TODO fuzz test position_* methods
...
test_that("position_max always have legal outputs()", {
  ...
  orders <- plyr::ldply(seq_len(df_length), function(pos){
    plyr::ldply(1:10, function(rep){
      setNames(position_max(df, col = "A", position = pos, shuffle_members = TRUE)$A,
               seq_len(df_length))
    }) %>%
...
test_that("position_min always have legal outputs()", {
  ...
  orders <- plyr::ldply(seq_len(df_length), function(pos){
    plyr::ldply(1:10, function(rep){
      setNames(position_min(df, col = "A", position = pos, shuffle_members = TRUE)$A,
               seq_len(df_length))
    }) %>%
      dplyr::mutate(pos = pos)
  }) %>% dplyr::distinct()
